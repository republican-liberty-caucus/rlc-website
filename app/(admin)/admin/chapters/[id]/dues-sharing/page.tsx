...
export default function ChapterDuesSharingPage() {
  ...
  const [childChapters, setChildChapters] = useState<{ id: string; name: string; chapter_level: string }[]>([]);

  const fetchConfig = useCallback(async () => {
    try {
      const res = await fetch(`/api/v1/admin/chapters/${chapterId}/split-config`);
  ...
  useEffect(() => {
    fetchConfig();
    fetchChildChapters();
  }, [chapterId, fetchConfig]);

  async function fetchChildChapters() {
    // Get chapters that could be recipients (self + descendants)
    const res = await fetch(`/api/v1/admin/chapters/${chapterId}/split-config`);
    // For now, we just provide an input for chapter ID
    ...
    try {
      const res = await fetch(`/api/v1/admin/chapters/${chapterId}/split-config`, {
        ...
        body: JSON.stringify({
          ...
          isActive,
          rules: rules.map((r) => ({
            recipientChapterId: r.recipientChapterId,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Configure how membership dues are split between this chapter and its sub-chapters"
      />
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              >
                ...
                <p className="text-sm text-muted-foreground">
                  National distributes to all sub-chapters per your configured rules
                </p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="text-sm text-muted-foreground">
                  State receives full remainder and handles sub-chapter distribution internally
                </p>
              </button>
            </div>
          </div>
      ...
      {disbursementModel === 'national_managed' && (
        <Card>
          ...
          <CardContent>
            {rules.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No rules configured. The full remainder after National&apos;s $15 fee will go to this chapter.
              </p>
            ) : (
                ...
                {rules.map((rule, index) => (
                  <div key={index} className="flex items-center gap-3 rounded-lg border p-3">
                    <div className="flex-1">
                      <Label className="text-xs">Recipient Chapter ID</Label>
                      <Input
                        value={rule.recipientChapterId}
                        onChange={(e) => updateRule(index, 'recipientChapterId', e.target.value)}
                        placeholder="Chapter UUID"
                        className="mt-1"
                      />
                      {rule.chapter_name && (
                        <p className="mt-1 text-xs text-muted-foreground">{rule.chapter_name}</p>
                      )}
                    </div>
                    <div className="w-28">
