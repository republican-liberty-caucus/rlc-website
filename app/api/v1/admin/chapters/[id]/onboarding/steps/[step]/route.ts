...
export async function GET(
  ...
  const { id: chapterId, step } = await params;
        ...
        for (const officer of meetingData.officers) {
          if (officer.member_id) {
            ...
            const { error: upsertError } = await (supabase as any).from('rlc_officer_positions').upsert({
              ...
              onConflict: 'member_id,chapter_id,title,committee_name',
            });
            if (upsertError) {
              logger.error('Error upserting officer position:', { officer, chapterId, error: upsertError });
