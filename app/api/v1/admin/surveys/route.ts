...
export async function POST(request: Request) {
  ...
  if (surveyError) {
    if (surveyError.code === '23505') {
      return apiError('A survey with this slug already exists', ApiErrorCode.CONFLICT, 409);
    }
    logger.error('Error creating survey:', surveyError);
    return apiError('Failed to create survey', ApiErrorCode.INTERNAL_ERROR, 500);
  }
  ...
  if (questionsError) {
    ...
    return apiError('Survey created but questions failed', ApiErrorCode.INTERNAL_ERROR, 500);
  }
